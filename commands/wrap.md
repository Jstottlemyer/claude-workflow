...
```bash
if [ -f graphify-out/graph.json ] && [ -x ~/Projects/MonsterFlow/scripts/dashboard-append.sh ]; then
...
```bash
python3 ~/Projects/MonsterFlow/scripts/session-insights.py 2>/dev/null || true
```
...
```bash
python3 ~/Projects/MonsterFlow/scripts/insights-parser.py 2>/dev/null || true
```
